...
//デザイン上のクラス名とjs操作用のすラス名は分けること！

//『やること』
//1. TODOを追加　を押下した際にタスクを追加する
//2. TODOタスクのアイコンを押下した際にタスクをDONEにする
//3.　DONEタスクのアイコンを押下した際にタスクをTODOタスクにもどす
...
$('.js-add-todo').on('click', function(e){
    e.preventDefault();//イベントをキャンセルしている（formのボタンがsubmitされてしまう）
    ...
    '<i class="fa-regular fa-square icon-check js-click-done" aria-hidden="true"></i>' + 
    ...
    $('.js-todo_list').prepend(listItem);// prependで直下のDOMを取る
});
...
// 4. クリックしたDOM（アイコン）からたどって、 list__itemのDOMを取得
// 5. list__itemのクラス名をdoneのものに変更する

$(document).on('click', '.js-click-done', function(){ //$(document)と書くことで後から追加したDOMにもイベントをセットできる

    $(this).removeClass('fa-regular').addClass('fa').
    ...
    // parent　は１階層上(親要素)の〜という意味
    // closest　は一番近い親要素の〜という意味
});
...
$(document).on('click', '.js-click-todo', function(){
    ...
    $(this).addClass('fa-regular').removeClass('fa').
    addClass('fa-square').removeClass('fa-check-square')
...
//2. クリックしたDOM（アイコン）からたどって、list__itemのDOMを取得し、削除する
...
$(document).on('keyup', '.js-todo_list-editForm', function(e){
    ...
    if(e.keyCode === 13 && e.shiftkey === true){
        ...
        //data即成を操作する場合、data()とattr()でのやり方がある
        //data()は、jQueryオブジェクト内のdataに対しての操作になる
        //attr()は、htmlオブジェクト（DOM）に対しての操作になる
    }
});

//【6.　検索エリアに値を入力した際にタスクの中なら値にマッチするタスクのみ表示させる】
...
$('.js-search').on('keyup', function(){
    ...
    $('.js-todo_list-item').show().each(function(i, elm) {
        ...
        var regexp = new RegExp('^' + searchText + '[a-zA-Z0-9]*');
